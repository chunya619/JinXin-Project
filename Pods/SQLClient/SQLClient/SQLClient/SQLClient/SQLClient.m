	 ...
	 completion:(nullable void(^)(BOOL success))completion
{
	...
	[self.workerQueue addOperationWithBlock:^{
		 ...
		 */
        self->_password = strdup([password UTF8String]);
        ...
        DBSETLUSER(self->_login, [username UTF8String]);
        DBSETLPWD(self->_login, self->_password);
        DBSETLHOST(self->_login, [host UTF8String]);
        DBSETLCHARSET(self->_login, [self.charset UTF8String]);
		
		//Set login timeout
		dbsetlogintime(self.timeout);
		
		//Connect to database server
        self->_connection = dbopen(self->_login, [host UTF8String]);
        if (!self->_connection) {
			[self connectionFailure:completion];
			return;
		}
		...
		if (database) {
            self->_returnCode = dbuse(self->_connection, [database UTF8String]);
            if (self->_returnCode == FAIL) {
				[self connectionFailure:completion];
				return;
			}
		}
...
- (void)execute:(nonnull NSString*)sql completion:(nullable void(^)(NSArray* _Nullable results))completion
{
	...
	[self.workerQueue addOperationWithBlock:^{
        ...
        self->_returnCode = dbcmd(self->_connection, [sql UTF8String]);
        if (self->_returnCode == FAIL) {
			[self executionFailure:completion];
			return;
		}
        ...
        self->_returnCode = dbsqlexec(self->_connection);
        if (self->_returnCode == FAIL) {
			[self executionFailure:completion];
			return;
		}
        ...
        while ((self->_returnCode = dbresults(self->_connection)) != NO_MORE_RESULTS)
		{
            if (self->_returnCode == FAIL) {
				[self executionFailure:completion];
				return;
            ...
            self->_numColumns = dbnumcols(self->_connection);
            ...
            self->_columns = calloc(self->_numColumns, sizeof(struct COLUMN));
            if (!self->_columns) {
				[self executionFailure:completion];
				return;
			}
            ...
            for (column = self->_columns; column - self->_columns < self->_numColumns; column++)
			{
				//Get column number
                int c = column - self->_columns + 1;
				
				//Get column metadata
                column->name = dbcolname(self->_connection, c);
                column->type = dbcoltype(self->_connection, c);
                column->size = dbcollen(self->_connection, c);
				...
				//Bind column data
                self->_returnCode = dbbind(self->_connection, c, bindType, column->size, column->data);
                if (self->_returnCode == FAIL) {
					[self executionFailure:completion];
					return;
				}
                ...
                self->_returnCode = dbnullbind(self->_connection, c, &column->status);
                if (self->_returnCode == FAIL) {
					[self executionFailure:completion];
					return;
				}
            ...
            while ((rowCode = dbnextrow(self->_connection)) != NO_MORE_ROWS)
			{
				//Check row type
				switch (rowCode)
				{
					...
					case REG_ROW:
					{
                        ...
                        NSMutableDictionary* row = [[NSMutableDictionary alloc] initWithCapacity:self->_numColumns];
							...
							if (column->status != -1)
							{
								switch (column->type)
								{
									...
									case SYBMONEY: //Monetary data from -922,337,203,685,477.5808 to 922,337,203,685,477.5807
									{
                                        ...
                                        dbconvert(self->_connection, SYBMONEY, column->data, sizeof(SYBMONEY), SYBCHAR, _value, -1);
									...
									{
										DBDATEREC2 _value;
                                        dbanydatecrack(self->_connection, &_value, column->type, column->data);
									...
									{
										DBDATEREC _value;
                                        dbdatecrack(self->_connection, &_value, (DBDATETIME*)column->data);
